...
class ValidateForm{
    ...
    ValorTodoR = () => {
        let cont = 0;
        for (let i = 0; i < this.valorTodo.length; i++) {
            if(!this.valorTodo[i]){
                cont++;
            }
        }
    ...
    ValidarVacio = (o,i) =>{
        ...
        if(oD.value == ""){
            this.ValidarMostrar(o,"Es obligatorio ingresar el campo.",this.campoMensaje[i],false);
            contGlobalVacio = false;
            this.valorTodo[i] = false;
        }else{
            this.ValidarMostrar(o,"",this.campoMensaje[i],true);
            contGlobalVacio = true;
            this.valorTodo[i] = true;
        }
    ...
    ValidarLongitud = (obj,i) =>{
        ...
        let exacto = this.ValidarUndefinido(obj.longitud.exacto) ? obj.longitud.exacto : false;
            if(min == 0 && exacto){
                if(lg == max){
                    this.ValidarMostrar(obj.campo,"",this.campoMensaje[i],true);
                    contGlobalLongitud = true;
                    this.valorTodo[i] = true;
                }else{
                    this.ValidarMostrar(obj.campo,"El campo debe tener "+max+" caracter(es).",this.campoMensaje[i],false);
                ...
                if(lg >= min  && lg<= max){
                    ...
                    this.ValidarMostrar(obj.campo,mensaje,this.campoMensaje[i],false);
                    contGlobalLongitud = false;
                    this.valorTodo[i] = false;
                }            
            }
            ...
            if(v == 1){
                ...
                if(indiceNuevo != -1){
                   ...
                   this.valorTodo[i] = true; 
                    ...
                    this.ValidarMostrar(o,"Ingrese un correo valido"+o,this.campoMensaje[i],false);  
    ...
    ValidarContrasena = (o,o1,i) => {
        ...
        if(texto != texto1){
            ...
            this.ValidarMostrar(o,"asd",this.campoMensaje[i],false);
            this.ValidarMostrar(o1,mensajeContra,this.campoMensaje[i],false);
            
            contGolbalIgual = false;
            this.valorTodo[i] = false;
        }else{
            this.ValidarMostrar(o,"",this.campoMensaje[i],true);
            this.ValidarMostrar(o1,"",this.campoMensaje[i],true);
    ...
    Formulario = () => {
        var vf = this;
        let oSub =this.objSub;
        document.getElementById(this.formulario).addEventListener("submit",function(e){
            e.preventDefault();
            if(vf.ValidarFormulario()){
                vf.resultado = true;
                vf.CambiarDis(oSub,false);
            }else{
                vf.resultado = false;
                vf.CambiarDis(oSub,true);
            }
            
        });
    }
    ValidarIniciar = () => {
        let form = document.getElementById(this.formulario);
        var is = form.querySelector("input[type=submit]");
        var bs = form.querySelector("button[type=submit]");
        this.objSub = is != null ? is : bs;
        let oSub = this.objSub;
        ...
        for (let i = 0; i < obtenerObjeto.length; i++) {
            ...
            this.campoMensaje[i] = campoD.parentNode.insertBefore(span,campoD.nextSibling)
            this.campoMensaje[i].classList.add("campo-error");
